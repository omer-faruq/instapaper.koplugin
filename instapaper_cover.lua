--[==[
Designed EPUB cover for Instapaper articles.
...
local function decodeToFill(data, mimetype, src_w, src_h, box_w, box_h)
    ...
    if not bb then
        ...
        if not ok or not res then
            logger.info("InstapaperCover: image decode failed", mimetype, res)
            return nil
        end
        bb = res
...
function InstapaperCover.build(title, author, site, image, out_path)
    ...
    if not ok then
        logger.warn("InstapaperCover: build failed", png)
        return nil, "cover_build_error"
    end
    return png, err
end

return InstapaperCover
